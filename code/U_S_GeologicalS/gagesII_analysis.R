...
sites <- read_csv("data/nesites.csv")

site_nums_df <- read_csv("data/nesites.csv")%>% # read sites list into R
...
basinID <- read.table("data/U_S_GeologicalS/Dataset1_BasinID/BasinID.txt",sep=",",header=T)
...
damRemoval_analysisBasin <- damRemoval %>% 
  ...
  tally() %>% # count number of dams removed per river_basin per given year (given by n)
...
damRemoval_analysisYear <- damRemoval %>% 
  ...
  tally() %>% #count number of dams removed each year
...
damRemoval_analysisSTAID <- damRemoval %>% 
  ...
  tally() %>% # count number of dams removed from each STAID per year
  dplyr::rename(num_dams = n) %>% 
  arrange(desc(num_dams)) %>% 
...
damRemoval_trends <- damRemoval %>% 
  left_join(read_csv("results/tables/rbi_kendall_senSlope_1971_2020.csv"), by = c("STAID" = "site_no")) %>% # join with trend analysis df
  filter(p.value < 0.05) %>% # filter for sites with significant p values
  mutate(highlight_flag = ifelse(`estimates.Sen's slope` > 0, "upward", "downward")) # add variable to classify trends 

  

# change variable orientation (create dataframe with WaterYear, STAID and rbi_val)
rbiWy_dfAll_pivotLong <- read_csv("results/tables/rbiWy_dfAll.csv") %>% 
...
damsRemovedAtSites <- site_nums_df %>% 
  ...
  full_join(read_csv("results/tables/rbi_kendall_senSlope_1971_2020.csv"), by = c("STAID" = "site_no")) %>% # join with trend analysis df
...
sites_corodinates_df = sitedata(site_nums)
...
gageSitesDamRemoval <- sites_corodinates_df %>% 
 ...
 # calculate distance between dam and gauge station
...
imperv <- read.table("data/U_S_GeologicalS/Dataset4_Imperviousness-Canopy/imperv_NLCD_2001-2011.txt",sep=",",header=T) %>% 
  left_join(read.table("data/U_S_GeologicalS/Dataset4_Imperviousness-Canopy/imperv_NWALT_1974-2012.txt",sep=",",header=T)) %>% 
  filter(STAID %in% sites$STAID) %>% #filter for study sites 
...
reference_sites <- imperv_pivoted %>% 
  ...
  full_join(read_csv("results/tables/rbi_kendall_senSlope_1971_2020.csv"), by = c("STAID" = "site_no")) %>% 
...
developedLandUse_NLCD <- read.table("data/U_S_GeologicalS/Dataset5_LandUse/LandUse_NLCD_2001.txt",sep=",",header=T) %>% 
  left_join(read.table("data/U_S_GeologicalS/Dataset5_LandUse/LandUse_NLCD_2006.txt",sep=",",header=T)) %>% 
  left_join(read.table("data/U_S_GeologicalS/Dataset5_LandUse/LandUse_NLCD_2011.txt",sep=",",header=T)) %>% 
  filter(STAID %in% sites$STAID) %>% #filter for study sites 
  select(STAID, contains(c("21", "22", "23", "24"))) %>% # select only the developed land variables
  mutate(STAID = as.character(paste0("0", STAID))) %>% 
  left_join(basinID) %>% 
  rowwise() %>% 
  dplyr::mutate(`2001` = rowSums(across(starts_with("NLCD01")), na.rm = T), # total developed land in 2001
...
imperv_developed <- developedLandUse_NLCD %>% 
  #select(STAID, developed_percent_diff) %>% 
  left_join(imperv, by = c("STAID" = "STAID")) %>%
  left_join(read_csv("results/tables/rbi_kendall_senSlope_1971_2020.csv"), by = c("STAID" = "site_no")) %>%
  #select(-ends_with(".y")) %>% 
  select(1:6, 22, 36:41) %>% 
...
trends_rbi <- sites %>% 
  ...
  left_join(read_csv("results/tables/rbi_kendall_senSlope_1971_2020.csv"), by = c("STAID" = "site_no")) %>% # join trend analysis results
  mutate(trend_type = ifelse(`estimates.Sen's slope` > 0, "high", "low"), # split trends into high(>0) and low(<0)
         lat = as.numeric(LAT_GAGE),
         long = as.numeric(LNG_GAGE)) %>% 
...
USGS_sites <- readxl::read_excel("data/U_S_GeologicalS/USGS-sites.xlsx") %>% 
  right_join(trend_analysis_df, by = c("STAID" = "site_no")) %>% 
  mutate(trend_type = ifelse(`estimates.Sen's slope` < 0, "negative", "positive"),
...
sitesTrends <- USGS_sites %>% 
  ...
  dplyr::rename(negativeTrends = negative,
...
siteTrendsSignificance <- USGS_sites %>% 
  ...
  pivot_wider(names_from = sigTrend,
              values_from = totalSites) %>% 
  dplyr::rename("negativeTrend - significant" = "significant - negative",
