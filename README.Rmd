...
To reproduce this analysis you will need to install the following packages:
...
1) Run **code/rbi_Values_Code/rbi_vals.R** to download USGS daily discharge data and calculate a Richards-Baker Index for sites specified in **/data/nesites.csv** over a given time frame. Here we use the `rbi_df`  from the [`easyrbi`](https://github.com/amutaya/easyrbi) R package.

2) Run **code/Trend_Analysis_Code/trends.R** to calculate the Mann-Kendall and Sens-Slope values 

3) Run **code/Trend_Analysis_Code/trend_analysis_visuals_summary.R** to generate scatter plots showing flashiness trends.

#### Land Use
4) Run **code/U_S_GeologicalS/gagesII_analysis.R** to analyse dam removals, changes in imperviousness and developed land use.

5) Run **code/U_S_GeologicalS/gagesII_vis.R** to visualize relationship between changes in landuse and stream flashiness.
